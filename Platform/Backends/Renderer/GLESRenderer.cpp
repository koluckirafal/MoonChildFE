...
bool GLESRenderer::BuildSharpBilinearProgram()
{
...
bool GLESRenderer::Init(IWindow* hostWindow)
{
    Window = hostWindow;
    ...
    if (!Surface->LoadOpenGLESFunctions())
    {
        printf("OpenGL ES function loading failed!\n");
        Window = nullptr;
        Surface = nullptr;
        return false;
    }
    ...
    {
        printf("OpenGL ES 2.0 shader program initialization failed!\n");
        Destroy();
        return false;
    }
...
void GLESRenderer::SetGlViewport(int windowWidth, int windowHeight, int bufferWidth, int bufferHeight)
{
...
void GLESRenderer::DrawFrame(const unsigned char* rgbaPixels, int width, int height)
{
    Surface->MakeCurrent();
...
void GLESRenderer::DisplaySetVSync(bool enabled)
{
